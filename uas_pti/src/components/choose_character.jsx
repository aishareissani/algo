...
function ChooseCharacter() {
  ...
  const arrowButtonBaseClass = "p-2 rounded-[20px] text-white text-[40px] leading-none transition-colors duration-300 select-none hover:text-emerald-400";
  ...
  return (
    <div className="character-screen bg-loading-screen fixed top-0 left-0 w-screen h-screen bg-cover bg-center bg-no-repeat flex flex-col items-center pt-16 sm:pt-20 z-[1000] px-4 overflow-y-auto">
      ...
      <div className="pt-[20px] mt-10 sm:mt-12 flex flex-col items-center gap-5 w-[220px]">
        <input type="text" placeholder="Enter your name" value={name} onChange={(e) => setName(e.target.value)} className="glass-input font-pressstart text-sm py-3 px-5 text-center text-gray-800 placeholder-gray-500 w-full" />
        <div className="pt-[20px]">
          <button
            disabled={!name.trim()}
            className={
              "leading-[1.5] nextButton font-pressstart text-[15px] rounded-[20px] py-[15px] px-[20px] sm:px-[40px] cursor-pointer border-none transition-all duration-500 shadow-btn-default hover:scale-110 hover:shadow-btn-hover animate-fadeInButton delay-[2000ms] w-full " +
